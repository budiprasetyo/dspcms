...
// class paging untuk halaman peraturan (menampilkan peraturan) 
class PagingPeraturan{
	// Fungsi untuk mencek halaman dan posisi data
	function cariPosisi($batas){
		if(empty($_GET['halperaturan'])){
			$posisi=0;
			$_GET['halperaturan']=1;
		}
		else{
			$posisi = ($_GET['halperaturan']-1) * $batas;
		}
		return $posisi;
	}
	...
	function navHalaman($halaman_aktif, $jmlhalaman){
		...
		if($halaman_aktif > 1){
			$prev = $halaman_aktif-1;
			$link_halaman .= "<a href=halperaturan-1.html><< First</a> | 
							<a href=halperaturan-$prev.html>< Prev</a> | ";
		}
		else{ 
			$link_halaman .= "<< First | < Prev | ";
		}
		...
		for ($i=$halaman_aktif-2; $i<$halaman_aktif; $i++){
		  if ($i < 1)
			continue;
			  $angka .= "<a href=halperaturan-$i.html>$i</a> | ";
		  }
			  $angka .= " <b>$halaman_aktif</b> | ";
			...
			if($i > $jmlhalaman)
			  ...
			  $angka .= ($halaman_aktif+2<$jmlhalaman ? " ... | <a href=halperaturan-$jmlhalaman.html>$jmlhalaman</a> | " : " ");
		...
		if($halaman_aktif < $jmlhalaman){
			$next = $halaman_aktif+1;
			$link_halaman .= " <a href=halperaturan-$next.html>Next ></a> | 
							 <a href=halperaturan-$jmlhalaman.html>Last >></a> ";
		}
		else{
			$link_halaman .= " Next > | Last >>";
		}
