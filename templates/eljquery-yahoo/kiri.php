...
elseif ($reqGet->getVarAlpha('module',9)=='hasilcari'){
		  echo "<div id='content'>          
               <div id='content-detail'>";
...
// Modul profil pegawai
elseif($_GET['module']=='profilpegawai'){
		
	echo "<div id='content'>
			<h3>>> <i><u>Profil Pegawai Direktorat Sistem Perbendaharaan</u></i></h3>";
			$pegawai	= mysql_query("SELECT id_pegawai,nama,nip,gambar,subdit,jabatan,keterangan FROM profilpegawai ORDER BY subdit,jabatan");
			echo "<table border='0' cellpadding='7'>";
			while($p	= mysql_fetch_array($pegawai)){
				if($p['jabatan'] == "1"){
					echo "<tr bgcolor='#5288DB'>";
				}
				elseif($p['jabatan'] == "2"){
					echo "<tr bgcolor='#9DA7B2'>";
				}
				elseif($p['jabatan'] == "3"){
					echo "<tr bgcolor='#A4BBF2'>";
				}
				elseif($p['jabatan'] == "4"){
				...
				elseif($p['jabatan'] == "5"){
					...
					echo "<td><span class=image><img src='foto_pegawai/$p[gambar]'></span></td>
						<td><b>Nama
							<br />NIP
							<br />Jabatan
							<br />Sub Direktorat</b>
					...
					{
						echo "<br />&nbsp;&nbsp;<b><font color='#0000FF'>Programmer ".$p[keterangan]."</font></b></td>";
					}
					echo "</tr>";
			}
			echo "</table>
...
elseif ($_GET['module']=='semuadownload'){
  ...
  // Tampilkan semua download
 	$sql = mysql_query("SELECT * FROM download  
                      ORDER BY lingkup_aplikasi,kategori_aplikasi,id_download DESC LIMIT $posisi,$batas");		 

  echo "<ul>"; 
   $lingkup_aplikasi = null;  
   while($d=mysql_fetch_array($sql)){
      echo "<br /><li>";
				if($d['lingkup_aplikasi'] != $lingkup_aplikasi){
					$lingkup_aplikasi = $d['lingkup_aplikasi'];
					switch($lingkup_aplikasi)
					{
						case 1:
						echo "<hr /><h3>APLIKASI KANWIL</h3><hr />";
						break;
						case 2:
						echo "<hr /><h3>APLIKASI KPPN</h3><hr />";
						break;
						case 3:
						echo "<hr /><h3>APLIKASI SATKER</h3><hr />";
						break;
					}
				}
	   echo"
			</li>
			<li><b>APLIKASI " . strtoupper($d[kategori_aplikasi]) . "</b></li>
			<li><a href='downlot.php?file=$d[nama_file]'>$d[judul]</a> ($d[hits])</li>
			<li>$d[deskripsi]</li>";
	 }
...
// Modul aplikasi kanwil
elseif ($_GET['module']=='aplikasikanwil'){
  echo "<div id='content'>          
          <div id='content-detail'>";
  echo "<span class=judul_head>&#187; <b>Download</b></span><br /><br />"; 
  $p      = new Paging5;
  ...
  $posisi = $p->cariPosisi($batas);
  // Tampilkan semua download
 	$sql = mysql_query("SELECT * FROM download WHERE lingkup_aplikasi=1
                      ORDER BY lingkup_aplikasi,kategori_aplikasi,id_download 
  ...
  $jmldata     = mysql_num_rows(mysql_query("SELECT * FROM download WHERE lingkup_aplikasi=1"));
  $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
  $linkHalaman = $p->navHalaman($_GET[haldownload], $jmlhalaman);
...
// Modul aplikasi kppn
elseif ($_GET['module']=='aplikasikppn'){
  echo "<div id='content'>          
          <div id='content-detail'>";
  echo "<span class=judul_head>&#187; <b>Download</b></span><br /><br />"; 
  $p      = new Paging5;
  ...
  $posisi = $p->cariPosisi($batas);
  // Tampilkan semua download
 	$sql = mysql_query("SELECT * FROM download WHERE lingkup_aplikasi=2
                      ORDER BY lingkup_aplikasi,kategori_aplikasi,id_download 
                      DESC LIMIT $posisi,$batas");		 
  ...
  $jmldata     = mysql_num_rows(mysql_query("SELECT * FROM download WHERE lingkup_aplikasi=2"));
  $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
  $linkHalaman = $p->navHalaman($_GET[haldownload], $jmlhalaman);
...
// Modul aplikasi instansi
elseif ($_GET['module']=='aplikasiinstansi'){
  echo "<div id='content'>          
          <div id='content-detail'>";
  echo "<span class=judul_head>&#187; <b>Download</b></span><br /><br />"; 
  $p      = new Paging5;
  ...
  $posisi = $p->cariPosisi($batas);
  // Tampilkan semua download
 	$sql = mysql_query("SELECT * FROM download WHERE lingkup_aplikasi=3
                      ORDER BY lingkup_aplikasi,kategori_aplikasi,id_download 
                      DESC LIMIT $posisi,$batas");		 
  ...
  $jmldata     = mysql_num_rows(mysql_query("SELECT * FROM download WHERE lingkup_aplikasi=3"));
  $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
  $linkHalaman = $p->navHalaman($_GET[haldownload], $jmlhalaman);
...
elseif($reqGet->getVarAlpha('module',9) == "peraturan"){
  echo "<div id='content'>          
          <div id='content-detail'>
			<div id='form'>
			<form method='post' name='f_aturan' action='" . htmlentities($_SERVER[PHP_SELF]) . "'>
			 <fieldset>
			  <legend><b>Pencarian Peraturan</b></legend><br />
				
				<input type='checkbox' name='jenaturanCek' />
				<label>Jenis Peraturan</label>
					<select name='jenaturan'>
						<option value='' selected='selected'>-- Pilih Jenis Peraturan --</option>";
							$qPeraturan	= mysql_query("SELECT DISTINCT * FROM jenisperaturan");
				...
				<input type='checkbox' name='tahunCek' />
				<label>Tahun Peraturan</label>
					<select name='tahun'>";
						$q		= "SELECT DISTINCT idperaturan,tglaturan FROM peraturan GROUP BY left(tglaturan,4)";
						$qTahun	= mysql_query($q)or die(mysql_errno);
						echo "<option value='' selected='selected'>-- Pilih Tahun --</option>";
						
						while($rTahun	= mysql_fetch_object($qTahun)){
							if(mysql_num_rows($qTahun) < 1)
							{
								echo "Belum ada peraturan yang di-<i>upload</i>";
							}
							else
							{
								echo "<option value='" . substr($rTahun->tglaturan,0,4) ."'>" . substr($util->dateConvert($rTahun->tglaturan),-4) . "</option>";
							}
						}
					echo "</select>
				
				<input type='checkbox' name='klasifikasiCek' />
				<label>Klasifikasi Peraturan</label>
					<select name='klasifikasi'>
						<option value='' selected='selected'>-- Pilih Klasifikasi Peraturan --</option>";
							$qKlasPeraturan	= mysql_query("SELECT DISTINCT * FROM klasifikasiperaturan ORDER BY idklasifikasi");
							while($rKlasPeraturan	= mysql_fetch_object($qKlasPeraturan)){
							echo "<option value='$rKlasPeraturan->klasifikasi'>" . $rKlasPeraturan->klasifikasi . "</option>";
							}
					echo "<option value></option>
					</select>
			
				<input type='checkbox' name='noaturanCek' />
				<label>Nomor Peraturan</label>
					<input type='text' name='noaturan' id='noaturan' maxlength='75' onkeypress='return handleEnter(this, event)' onkeyup=\"moveOnMax(this,'norak')\" />
				
				
				
				<input type='checkbox' name='tentangCek' />
				<label>Tentang</label>
					<input type='text' name='tentang' id='tentang' maxlength='200' />
					
				<input type='submit' name='sf_aturan' value='Cari' />
			  </fieldset>
			</form>
			</div>
			<br />
			<br />";
			$sLastposting = "SELECT noaturan,uraianaturan,tglposting,file FROM peraturan ORDER BY tglposting DESC LIMIT 10";
			$qLastposting = mysql_query($sLastposting);
			echo "<div id='table'>
			<table>
			<tr>
				<th colspan='2'><p><b>Sepuluh peraturan terakhir yang diposting:</b></p></th>
			</tr>";
			$no = 1;
			$odd	= "#CBDEF7";
			$even	= "#e0e0f0";
			while($rLastposting = mysql_fetch_object($qLastposting))
			{
				if($no % 2 == 0){$color = $odd;}
				else{$color = $even;}
				echo "
				<tr bgcolor='".$color."'><td width='30px' align='center'>".$no."</td><td>Diposting tanggal ".$util->dateConvert($rLastposting->tglposting)." peraturan nomor: <b><a href='peraturan/$rLastposting->file' target='_blank'>".$rLastposting->noaturan."</a></b> tentang <b>".$rLastposting->uraianaturan."</b></li></td></tr>";
				$no++;
			}
			echo"</table>
			</div>
...
// Pencarian Peraturan
elseif($_POST['sf_aturan'] == "Cari"){
	
	$jenaturanCek 	= $_POST['jenaturanCek'];
	$jenaturan		= $_POST['jenaturan'];
	$noaturanCek	= $_POST['noaturanCek'];
	$noaturan		= $_POST['noaturan'];
	$tahunCek		= $_POST['tahunCek'];
	$tahun			= $_POST['tahun'];
	$klasifikasiCek	= $_POST['klasifikasiCek'];
	$klasifikasi	= $_POST['klasifikasi'];
	$tentangCek		= $_POST['tentangCek'];
	$tentang		= $_POST['tentang'];
	$countWordsTentang = $util->countWords($tentang);
	$tentang		= str_replace(" ","%",$tentang);
	$where 			= "";
	
	
	if($jenaturanCek !== "on" && $noaturanCek !== "on" && $tahunCek !== "on" && $klasifikasiCek !== "on" && $tentangCek !== "on")
	{
		echo "<script type='text/javascript'>
			alert('Berikan tanda cek pada salah satu cek box');
			window.location.href='peraturan.html';
			</script>";
	}elseif($jenaturanCek === "on" && $jenaturan == ""){
			echo "<script type='text/javascript'>
			alert('Berikan tanda cek dari cek box jenis aturan dan pilih jenis aturan');
			window.location.href='peraturan.html';
			</script>";
	}elseif($noaturanCek === "on" && $noaturan == ""){
			echo "<script type='text/javascript'>
			alert('Berikan tanda cek dari cek box nomor aturan dan isikan nomor aturan');
			window.location.href='peraturan.html';
			</script>";
	}elseif($tahunCek === "on" && $tahun == ""){
			echo "<script type='text/javascript'>
			alert('Berikan tanda cek dari cek box tahun dan pilih tahun');
			window.location.href='peraturan.html';
			</script>";
	}elseif($klasifikasiCek === "on" && $klasifikasi == ""){
			echo "<script type='text/javascript'>
			alert('Berikan tanda cek dari cek box klasifikasi dan pilih klasifikasi');
			window.location.href='peraturan.html';
			</script>";
	}elseif($tentangCek === "on" && $tentang == ""){
			echo "<script type='text/javascript'>
			alert('Berikan tanda cek dari cek box tentang dan isian kolom tentang');
			window.location.href='peraturan.html';
			</script>";
	}
		...
		if($jenaturanCek == "on"){
			$jenaturan;
			if(empty($where)){
				$where .= "jenaturan = '" . $jenaturan . "'";
			}
		}
		// where noaturan
		if($noaturanCek == "on"){
			if(empty($where)){
				$where .= " noaturan REGEXP '" . $noaturan . "'";
			}else{
				$where .= " AND noaturan REGEXP '$noaturan'";
			}
		}
		// where tahun
		if($tahunCek == "on"){
			if(empty($where)){
				$where .= "YEAR(tglaturan) = '$tahun'";
			}else{
				$where .= " AND YEAR(tglaturan) = '$tahun'";
			}
		}
		// where klasifikasi
		if($klasifikasiCek == "on"){
			if(empty($where)){
				$where .= "klasifikasi REGEXP '" . $klasifikasi . "'";
			}else{
				$where .= " AND klasifikasi REGEXP '" . $klasifikasi . "'";
			}
		}
		// where tentang
		if($tentangCek == "on"){
			if(empty($where)){
				$where .= "uraianaturan LIKE '%$tentang%'";
			}else{
				$where .= " AND uraianaturan LIKE '%$tentang%'";
			}
		}
		
	echo "<div id='content'>          
          <div id='content-detail'>
          <span class=judul_head>&#187; <b>Hasil Pencarian</b></span><br /><br /><br />
			<div id='table'>
			<table id='myTable' class='tablesorter'>
				<thead>
				<tr>
					<th width='15%'>No.& Tgl.Peraturan</th>
					<th>Tentang</th>
				...
				$q		= "SELECT noaturan,tglaturan,jenaturan,uraianaturan,klasifikasi,keterangan,file FROM peraturan WHERE " . $where . "ORDER BY jenaturan,noaturan,tglaturan";
				$qAtur	= mysql_query($q);
				$i		= 1;
				$odd	= "#CBDEF7";
				$even	= "#e0e0f0";
				while($rAtur	= mysql_fetch_object($qAtur)){
					// if($i % 2 == 0){$color = $odd;}
					// else{$color = $even;}
					$tglaturan	= $util->dateConvert($rAtur->tglaturan);
				
				echo"
				<tr>
					<td align='center'><b>" . $rAtur->noaturan . "<br />" . $tglaturan . "</b></td>
					<td>" . $rAtur->uraianaturan ."</td>
					<td align='center'>" . strtoupper($rAtur->klasifikasi) . "</td>
					<td>" . $rAtur->keterangan . "</td>
					<td><a href='peraturan/" . $rAtur->file . "' target='_blank'>" . $rAtur->file . "</a></td>
				</tr>";
				$i++;
				}
				echo "
		   ...
		   echo "</div>
		   <div id='pager' class='pager' style='top: 800px; postion: absolute;'>
			<form>
				<img class='first' src='jQuerySorter/addons/pager/images/first.png' />
				<img class='prev' src='jQuerySorter/addons/pager/images/prev.png' />
					<input class='pagedisplay' type='text' />
				<img class='next' src='jQuerySorter/addons/pager/images/next.png' />
				<img class='last' src='jQuerySorter/addons/pager/images/last.png' />
