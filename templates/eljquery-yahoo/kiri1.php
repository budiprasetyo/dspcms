...
elseif($_GET['module']=='profilpegawai'){
	...
	echo "<div id='content'>
			...
			$pegawai	= mysql_query("SELECT id_pegawai,nama,nip,gambar,subdit,jabatan FROM profilpegawai ORDER BY subdit,jabatan");
...
elseif($_GET['module'] == "forum"){
	echo "<script type='text/javascript'>
		window.location('http://www.perbendaharaan.go.id');
	</script>";
}

// Modul Peraturan
elseif($_GET['module'] == "peraturan"){
  echo "<div id='content'>          
          <div id='content-detail'>
			<div id='form'>
			...
			$sLastposting = "SELECT noaturan,uraianaturan,tglposting FROM peraturan ORDER BY tglposting DESC LIMIT 5";
			$qLastposting = mysql_query($sLastposting);

			echo "<p><b>Lima peraturan terakhir yang diposting:</b></p>
			<ol>";

			while($rLastposting = mysql_fetch_object($qLastposting))
			{
				echo "<li>Diposting tanggal ".$util->dateConvert($rLastposting->tglposting)." peraturan nomor: <b>".$rLastposting->noaturan."</b> tentang <b>".$rLastposting->uraianaturan."</b></li>";
			}
			echo"
			</ol>
		  </div>
	...
	echo "<div id='content'>          
          ...
          <span class=judul_head>&#187; <b>Hasil Pencarian</b></span><br /><br /><br />
			...
			<table>
				...
				$q		= "SELECT noaturan,tglaturan,jenaturan,uraianaturan,klasifikasi,keterangan,file FROM peraturan WHERE " . $where . "ORDER BY jenaturan,noaturan";
				$qAtur	= mysql_query($q);
				$i		= 1;
				$odd	= "#CBDEF7";
				$even	= "#e0e0f0";
				...
				$jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
				$linkHalaman = $p->navHalaman($_GET[halperaturan], $jmlhalaman);
				*/
				echo "</table>
				</div>";
			/*
